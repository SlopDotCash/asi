...
def _require_hyperparameter_coordinate(
    ...
    *,
    ...
    """
    host_name_top = _require_exact_str("name", name)

    def reject() -> NoReturn:
        host_name = host_name_top
        raise ValueError(
    ...
    if _type_identity_in(value_type, (tuple, frozenset)):
        if nesting_depth >= _MAX_HYPERPARAMETER_COORDINATE_NESTING:
            host_name_inner = _require_exact_str("name", name)
            raise ValueError(
                "param_extractor returned an over-nested coordinate for "
                f"configuration '{host_name_inner}'; coordinates support at most "
