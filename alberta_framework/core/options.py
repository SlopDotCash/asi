...
def _epsilon_greedy_action_probabilities(q_values: Array, epsilon: Array) -> Array:
    """Return epsilon-greedy probabilities matching the Gumbel tie-break selector.

    ``_select_action_epsilon_greedy*`` picks the greedy action with
    ``argmax(q + 1e-6 * Gumbel(0, 1))``, so the greedy component's
    probabilities are ``softmax(q / 1e-6)`` — near-tied values are broken by
    the Gumbel draw, not collapsed into an exact uniform tie. The importance
    ratio must use the same distribution or it scales traces with
    probabilities from a policy that did not generate the action (#2136).

    An exact tie (identical q values) still yields uniform greedy
    probabilities, because ``softmax`` of equal logits is uniform.
    """
    q = jnp.asarray(q_values, dtype=jnp.float32)
    n_actions = q.shape[0]
    eps = jnp.asarray(epsilon, dtype=jnp.float32)
    temperature = jnp.asarray(1e-6, dtype=jnp.float32)
    logits = q / temperature
    logits = logits - jnp.max(logits)  # numerical stability
    greedy_probs = jax.nn.softmax(logits)
    uniform = jnp.full((n_actions,), 1.0 / n_actions, dtype=jnp.float32)
    return eps * uniform + (1.0 - eps) * greedy_probs
