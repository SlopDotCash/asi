...
# Temperature of the Gumbel-max tie-break used by every intra-option/base
# ``argmax(q + temp * gumbel(...))`` selector.  By the Gumbel-max trick the
# greedy component of that behavior policy is ``softmax(q / temp)``, so the
# importance-ratio helper must reuse the exact same constant to model it; a
# separate value would let the sampled policy and its ratio model drift apart.
_GUMBEL_TIEBREAK_TEMPERATURE = 1.0e-6
...
def _select_action_epsilon_greedy(
    ...
    greedy = jnp.argmax(
        q_vals + _GUMBEL_TIEBREAK_TEMPERATURE * jr.gumbel(noise_key, (n_actions,))
    ).astype(jnp.int32)
...
def _select_action_epsilon_greedy_from_q_masked(
    ...
    key, explore_key, noise_key = jr.split(key, 3)
    noisy = q_vals + _GUMBEL_TIEBREAK_TEMPERATURE * jr.gumbel(noise_key, (n_actions,))
...
def _epsilon_greedy_action_probabilities(q_values: Array, epsilon: Array) -> Array:
    """Return epsilon-greedy probabilities of the Gumbel tie-break policy.

    The greedy component is the exact Gumbel-max distribution generated by the
    ``argmax(q + _GUMBEL_TIEBREAK_TEMPERATURE * gumbel(...))`` selectors, i.e.
    ``softmax(q / _GUMBEL_TIEBREAK_TEMPERATURE)``.  Subtracting ``max_q`` before
    dividing keeps the softmax numerically stable: large Q-gaps saturate to a
    near one-hot argmax instead of overflowing, while exact ties reduce to a
    uniform split.  The uniform epsilon-exploration term is then mixed in.
    """
    q = jnp.asarray(q_values, dtype=jnp.float32)
    n_actions = q.shape[0]
    eps = jnp.asarray(epsilon, dtype=jnp.float32)
    max_q = jnp.max(q)
    temp = jnp.asarray(_GUMBEL_TIEBREAK_TEMPERATURE, dtype=jnp.float32)
    greedy_probs = jax.nn.softmax((q - max_q) / temp)
