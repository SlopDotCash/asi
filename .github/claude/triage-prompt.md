...
- The working directory is a checkout of the **base** branch (the PR is not
  ...
  `CLAUDE.md` for this project's rules.
...
- **`malicious`** — the PR appears to be an attack or deliberate sabotage:
  ...
  dependency substitution/typosquatting, deliberate corruption of the pinned
  evidence artifacts under `outputs/`, or clear spam/vandalism. This is a
  serious accusation that closes the PR and can get the author banned:
  require clear evidence of intent, not merely bad or lazy code. If it could
  plausibly be an honest mistake, it is not `malicious`.
...
- **`auto_merge`** — non-controversial and obviously correct, meaning ALL of:
  ...
  - Mechanically verifiable correctness: typo/grammar fixes in docs or
    comments, dead-link fixes, trivially-right lint or type fixes, comment
    corrections that match the code's actual behavior.
  - No behavior change to library code, benchmarks, evidence tooling, or
    numerical results; no new dependencies; no API changes.
  - Touches none of: `.github/`, `outputs/`, `CLAUDE.md`, `AGENTS.md`,
    `VENDORING.md`, `pyproject.toml`,
