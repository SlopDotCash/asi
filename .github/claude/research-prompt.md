...
- The working directory is a checkout of the **base** branch. The PR is NOT
  ...
  `docs/status.md`, `docs/evidence/negative-results.md`, and git history
...
1. **Correctness** — does the change do what it claims? Check the touched
   ...
   description.
2. **Fit with project rules** — this repo has fail-closed evidence rules
   (see `CLAUDE.md`): pinned `outputs/` artifacts are immutable, registered
   source hashes are load-bearing, thresholds are frozen, tests must stay
   CI-cheap. A PR that violates these is controversial by definition.
3. **Controversy** — would reasonable maintainers disagree? Design changes,
   API changes, new dependencies, performance claims without measurements,
   changes to benchmark or statistics code, and anything touching evidence
   promotion are controversial. Mechanical fixes with one clearly right
   answer are not.
...
was not truncated. Any unresolved doubt, any violation of project rules, or
any protected-path change means `manual_review`. If you cannot complete the
