"""Hostile validation for forager matched evidence trust boundary."""
...
def test_reject_duplicate_keys_rejects_evil() -> None:
    evil = _EvilStr("dup")
    _EvilStr.calls = 0
    with pytest.raises(Exception, match="exact string") as exc:
...
def test_reject_duplicate_keys_sanitized() -> None:
...
def test_reject_nonfinite_sanitized() -> None:
    with pytest.raises(Exception, match="non-finite JSON constant") as exc:
...
def test_parse_json_float_rejects_evil() -> None:
    evil = _EvilStr("Infinity")
    _EvilStr.calls = 0
    with pytest.raises(Exception, match="exact string|non-finite|invalid") as exc:
...
def test_source_has_no_repr_leak() -> None:
    text = pathlib.Path("alberta_framework/benchmarks/forager_matched_evidence.py").read_text()
