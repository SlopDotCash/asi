"""Origin-complete: deep recurring-feature evidence JSON must fail closed before RecursionError."""
...
def test_load_recurring_feature_artifact_rejects_origin_recursion_fixture(
    tmp_path: Path,
) -> None:
    ...
    with pytest.raises(ValueError, match="JSON nesting limit"):
        load_recurring_feature_artifact(path)


def test_load_recurring_feature_artifact_rejects_first_over_depth_object(
    tmp_path: Path,
) -> None:
...
def test_load_recurring_feature_artifact_rejects_deep_array_nest(
    tmp_path: Path,
) -> None:
...
def test_load_recurring_feature_artifact_accepts_bounded_object(
    tmp_path: Path,
) -> None:
    path = tmp_path / "ok.json"
    path.write_text('{"schema": "x"}\n', encoding="utf-8")
    assert load_recurring_feature_artifact(path) == {"schema": "x"}
