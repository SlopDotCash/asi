"""Hostile-identity tests for dreaming config consumer."""

from __future__ import annotations

import pytest

from alberta_framework.core.dreaming import DreamingConfig, _copy_mapping
...
def test_copy_mapping_rejects_hostile_without_dispatch() -> None:
    ...
    with pytest.raises(ValueError):
        _copy_mapping(hostile, name="DreamingConfig")  # type: ignore[arg-type]
    assert _HostileMapping.calls == 0


def test_dreaming_from_config_rejects_hostile_without_dispatch() -> None:
    hostile = _HostileMapping({"warmup_steps": 100})
    _HostileMapping.calls = 0
    with pytest.raises(ValueError):
