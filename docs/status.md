...
| Agent composition | [Versioned L0 reference transaction contract](../alberta_framework/reference_agent.py) with immutable typed payloads, separate authorization/settlement/receipt/outcome records, explicit reset identities, and a fail-closed single-writer phase ledger covered by [retained contract tests](../tests/test_reference_agent_protocol.py); `PrototypeAgent`, explicit transition ownership, agent-only checkpoints, and Step/pipeline kernels | No concrete Prototype or robot adapter, aggregate life state or authoritative runner, whole-life checkpoint, exact-resume result, selected `reference-dev`, or uninterrupted L3 life exists; robot and Forager paths do not consume `PrototypeAgent`, whose closed-loop extended-action dispatch remains unresolved |
